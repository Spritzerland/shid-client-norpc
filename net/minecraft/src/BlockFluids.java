...
import net.minecraft.src.stever9487.hacks.Jesus;
...
public abstract class BlockFluids extends Block {
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World var1, int var2, int var3, int var4) {
		Jesus jesus = (Jesus) Client.hacks[15];
		if(jesus.isToggled()) {
			try{
				if(Client.mc.thePlayer.inWater && !Client.mc.thePlayer.isSneaking()) {
					Client.mc.thePlayer.func_424_C();
					return null;
				}else{
					return Client.mc.thePlayer.fallDistance < 2.0F && !Client.mc.thePlayer.isSneaking() ? super.getCollisionBoundingBoxFromPool(var1, var2, var3, var4) : null;
				}
			}catch(Exception ex) {
				return null;
			}
	...
	public float getBlockBrightness(IBlockAccess var1, int var2, int var3, int var4) {
		...
		return Client.hacks[2].isToggled() ? 1.3F : Math.max(var5, var6);
	}
