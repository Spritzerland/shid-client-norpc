...
public abstract class Entity {
	...
	public void moveEntity(double var1, double var3, double var5) {
		...
		if(this.noClip || (this instanceof EntityPlayer && Client.hacks[7].isToggled())) {
	...
	public Vec3D func_174824_e(float f) {
		if(f == 1.0F) {
			...
			double X = this.prevPosX + (this.posX - this.prevPosX) * (double)f;
			double Y = this.prevPosY + (this.posY - this.prevPosY) * (double)f + (double)this.getEyeHeight();
			double Z = this.prevPosZ + (this.posZ - this.prevPosZ) * (double)f;
